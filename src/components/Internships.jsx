      ...
      <section id="internships" className="py-20 select-none bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            ...
            >
              Real-world industry experience through structured internship programs
            </motion.p>
          </motion.div>
              ...
              >
                ...
                <div className="bg-linear-to-br from-cyan-500 to-purple-600 px-6 pt-6 pb-5 relative">
                  ...
                  <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-md mb-3 overflow-hidden">
                    ...
                    <div className="w-full h-full items-center justify-center" style={{ display: item.logo ? "none" : "flex" }}>
                  ...
                  <p className="text-white/80 text-xs font-semibold uppercase tracking-wide">{item.company}</p>
                  <h3 className="text-white text-md font-bold mt-0.5 leading-snug">{item.role}</h3>
                  <div className="absolute top-4 right-4 flex flex-col gap-1 items-end">
                    <span className="bg-white/20 text-white text-xs px-2 py-0.5 rounded-full backdrop-blur-sm">📄 Offer Letter</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full backdrop-blur-sm ${
                      item.completionCertImage ? "bg-white/20 text-white" : "bg-white/10 text-white/50"
                    }`}>
                      🎓 {item.completionCertImage ? "Certificate" : "Cert. Pending"}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mb-5">
                    ...
                    {item.skills.length > 3 && (
                      <span className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded-md">+{item.skills.length - 3} more</span>
                    )}
                  </div>
                  <motion.button
          ...
          {selectedInternship && (
            ...
            >
              ...
              >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 mt-1 text-white/75 text-xs flex-wrap">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="whitespace-nowrap">{selectedInternship.period}</span>
                        </div>
                ...
                <div className="overflow-y-auto flex-1" style={{ scrollbarWidth: "none", msOverflowStyle: "none", WebkitOverflowScrolling: "touch" }}>
                  <div className="p-5 md:p-6 space-y-5">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }}>
                      ...
                      <div className="rounded-2xl overflow-hidden shadow-lg border border-gray-200">
                        <img src={selectedInternship.offerLetterImage} alt="Internship Offer Letter" className="w-full h-auto object-contain bg-white" />
                      </div>
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.25 }}>
                      ...
                      {selectedInternship.completionCertImage ? (
                        <div className="rounded-2xl overflow-hidden shadow-lg border border-gray-200">
                          <img src={selectedInternship.completionCertImage} alt="Completion Certificate" className="w-full h-auto object-contain bg-white" />
                        </div>
                      ) : (
                        <div className="rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 p-8 text-center">
                          ...
                          <p className="text-gray-500 text-sm font-medium">Certificate Pending</p>
                          <p className="text-gray-400 text-xs mt-1">Will be added upon internship completion on April 7, 2026</p>
                        </div>
                      )}
                    </motion.div>

                    {/* About */}
                    <motion.div className="bg-linear-to-br from-gray-50 to-white p-4 rounded-xl border border-gray-100" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.3 }}>
                      ...
                      <p className="text-gray-700 leading-relaxed text-sm">{selectedInternship.description}</p>
                    </motion.div>

                    {/* Key Highlights */}
                    <motion.div className="bg-linear-to-br from-green-50 to-white p-4 rounded-xl border border-green-100" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.4 }}>
                      <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                      ...
                      <ul className="space-y-2">
                        {selectedInternship.achievements.map((a, i) => (
                          <motion.li key={i} className="flex items-start gap-3" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.5 + i * 0.1 }}>
                    ...
                    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.5 }}>
                      ...
                      <div className="flex flex-wrap gap-2.5">
                        {selectedInternship.skills.map((skill, i) => (
                          <motion.span key={i} className="px-4 py-2 bg-linear-to-r from-cyan-50 to-purple-50 text-cyan-700 text-sm font-medium rounded-lg border border-cyan-200 hover:border-cyan-300 transition-colors" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, delay: 0.6 + i * 0.05 }} whileHover={{ scale: 1.05 }}>
                            {skill}
                          </motion.span>
                        ))}
                      </div>
                    ...
                    {/* Additional Info */}
                    <motion.div className="bg-linear-to-br from-blue-50 to-white p-4 rounded-xl border border-blue-100" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.6 }}>
                      <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <p className="text-xs font-semibold text-gray-500 mb-1.5">Intern ID</p>
                          <p className="font-mono text-xs text-gray-900 bg-white px-3 py-2 rounded-lg border border-gray-200">{selectedInternship.internId}</p>
                        </div>
                        <div>
                          <p className="text-xs font-semibold text-gray-500 mb-1.5">Location</p>
                          <p className="text-xs text-gray-900 bg-white px-3 py-2 rounded-lg border border-gray-200">{selectedInternship.location}</p>
                        </div>
                        <div className="md:col-span-2">
                          <p className="text-xs font-semibold text-gray-500 mb-1.5">Organization</p>
                          <p className="text-xs text-gray-900 bg-white px-3 py-2 rounded-lg border border-gray-200">{selectedInternship.company} — ISO 9001 Certified, AICTE Approved</p>
                        </div>
                      </div>
                    </motion.div>
